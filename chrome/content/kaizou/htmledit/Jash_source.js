/**
* $Id: Jash_source.js,v 1.2 2008/01/24 19:30:01 corvoysier Exp $
* <pre>
...
function Jash() {
	...
	*/
	this.revision = "$Revision: 1.2 $".replace(/(\$|[A-Za-z]|\s|:)/g,'');
	this.version = "$Name:  $".replace(/\$|Name:|\s|REL_/g,'').replace(/_/g,'.');
	this.versionDate = "$Date: 2008/01/24 19:30:01 $";
	...
	*/
	this.main = function() {
		...
		*/
		...
		/* if this page has overridden the global event, delete it so that it can be restored */
		if(typeof event != "undefined") delete event;
	}
	...
	*/
	this.identifyNode = function(el,showDots) { 
		...
		for(var i = 0; i < el.attributes.length; i++) {
			if((this.browser == "ie" && el.attributes[i].specified === true) ||
			this.browser != "ie") {
				out.txt += " " + el.attributes[i].name; 
				out.txt += "=\"" + el.attributes[i].value + "\"";
			}
		}
...
Jash.History.prototype = {
	...
	*/
	getNextInput: function() {
		/* if position is still under history length */
